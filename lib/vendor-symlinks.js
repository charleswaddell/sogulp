...
var setup = function(err, progress, complete) {
	fs.readdirSync(paths.vendors).forEach(function(packageName, index) {
...
var teardown = function(err, progress, complete) {
	fs.readdirSync(paths.vendors).forEach(function(packageName, index) {
		...
		if (fs.existsSync(packageLinkPath) &&
			fs.existsSync(packageLinkPath+suffix) &&
			fs.lstatSync(packageLinkPath).isSymbolicLink() &&
			fs.lstatSync(packageLinkPath+suffix).isDirectory()) {

			fs.unlinkSync(packageLinkPath);
			fs.renameSync(packageLinkPath+suffix, packageLinkPath);
